...
def editusersettings():
    if not session.get('logged_in'):
        ...
        return render_template('settings.html', friends_ph=db.getfriends(),
                               ...
                               color=db.getcolors(), pages=db.googlepages())
...
def edit_settings():
    if not session.get('logged_in'):
        ...
        if str(request.form['pages']) != str(db.googlepages()):
            db.site.find_one_and_update({"pages": db.googlepages()},
                                        {"$set": {"pages": request.form['pages']}})
...
def manual():
    if not session.get('logged_in'):
        ...
        for x in lista.split('\r\n'):
            if x != '':
                kwlst.append(x)
        client = main.MainApp(kw=None, friends=db.getfriends(), competitors=db.getcompetitors(),pages=db.googlepages())
        client.resetitems()
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=52166,debug=True)
